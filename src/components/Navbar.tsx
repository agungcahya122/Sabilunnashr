...
const Navbar = () => {
  ...
  const logout = () =>
    axios
      .post("https://sabilun.promaydo-tech.com/api/auth/logout", null, {
        params: { token: token },
      })
      .then((resp) => {
        console.log(resp.data)
        ...
        <div className="modal">
          <div className="modal-box relative w-1/3 max-w-full bg-color4 text-center">
            ...
            <CustomButton
              ...
              id="btn-login"
              label="Logout"
              className="mt-4 rounded-2xl bg-color6 px-10 py-1 text-[16px] font-semibold text-white shadow-[1px_2px_4px_0px_rgba(0,0,0,0.5)] hover:bg-[rgb(0,140,255)] disabled:cursor-not-allowed disabled:bg-color2"
            />
          </div>
        </div>
      </div>
